    ...
    super(props);
     this.state = {
      ...
      idComercio: this.props.idComercio,
       ratio: 3,
      sourceImage: {uri: 'https://www.mako.guru/directorio/imagenes/informativos/sinInformativo.jpg'},
      hasImage: 0,
      data: null,
       ...
       if (this.props.item.imagen != null) {
           this.ImageURI = 'https://www.mako.guru/directorio/imagenes/informativos/'+this.state.idComercio+"/"+this.props.item.imagen;
          ...
          this.ImageURI = 'https://www.mako.guru/directorio/imagenes/informativos/sinInformativo.jpg';
          let imgPromo = {uri : this.ImageURI};
          this.setState({
            sourceImage : imgPromo,
            hasImage: 0,
...
componentWillReceiveProps(nextProps: Props) {
 ...
 Image.getSize( this.ImageURI, ( Width, Height ) =>
     ...
     if (this.props.item.imagen != null) {
          ...
          let imgPromo = {uri : 'https://www.mako.guru/directorio/imagenes/informativos/'+this.state.idComercio+"/"+this.props.item.imagen};
          this.setState({
            sourceImage : imgPromo,
            hasImage: 1,
          })
...
PreguntaEliminarImg( titulo){
  Alert.alert(
  'Estas seguro!',
  'Vas a eliminar la imagen para el informativo - '+titulo,
  [
    {text: 'Cancelar', onPress: () => console.log('nooooo'), style: 'cancel'},
...
uploadImage(nombre,imagen,eliminar){
	 RNFetchBlob.fetch('POST', 'https://www.mako.guru/listadosApp/uploadImgInfor.php', {
...
guardaUrl(nombre, imagen){
    ...
    return fetch('http://www.mako.guru/listadosApp/guardaUrlInfor.php',{
    method:'post',
    headers:{
      'Accept': 'aplication/json',
      'Content-Type': 'aplication/json',
            ...
            <View style={[styles.promo]}  >
                ...
                <View style={[styles.contPromo]}>
                  ...
                  <View style={[styles.txtPromo]}>
                    ...
                    <Text style={styles.txtFechas}> {'Termina el: '+ (item.fechaFin) } </Text>

                  <View style={[styles.btnAccion]}>


                    <TouchableOpacity style={[styles.btnEliminar]}  onPress={()=> this.actDesactInf(item.id,item.activo)}>
                      <Icon name={iconoActivo} size={20} color={'#4177F6'}/>
                      <Text style={styles.txtEliminar}> {lblActivo} </Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={[styles.btnEditar]} onPress={()=> Actions.EditarInfo({idInfo: item.id,idComercio: this.state.codigo, titulo: item.titulo, descrip: item.descripcion , fechaInicio: item.fechaInicio , fechaFin: item.fechaFin , activo: item.activo , colorCiudad: this.props.colorCiudad})}>
                      <Icon name="edit" size={20} color={'#79DB48'}/>
