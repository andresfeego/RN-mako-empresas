...
export default class EditarPromo extends Component {
  ...
  constructor(props) {
    super(props);
     this.state = {
      ...
      porciento: this.props.porciento,
      ...
      label: 'Las promociones que se encuentren inactivas seran activadas de forma automatica a su fecha de inicio.',
...
guardar(){
    ...
    this.setState({
              label: 'Guardando promocion...',
            })

    return fetch('http://www.mako.guru/listadosApp/EditarPromo.php',{
    method:'POST',
    headers:{
    ...
    body:JSON.stringify({
       idPromo: this.state.idPromo,
       idComercio: this.state.idComercio,
      porciento: this.state.porciento,
...
.catch((error) => {
 ...
 <View style={styles.container}>
  ...
  <ScrollView style={styles.formulario}>
        <Text style={[styles.txtId,{color: this.state.colorCiudad}]}>{'Editando promocion con id: '+this.state.titulo}</Text>
        ...
        <TextInput placeholder={'Porciento %'} placeholderTextColor="gray"  maxLength = {3} onChangeText={porciento => this.setState({porciento})} style={styles.txtFormu}>{this.state.porciento}</TextInput>
        ...
        {this.state.existencias == 1 ? 
        <View>
        <Text style={styles.txtId}>Hasta agotar existencias</Text>
          ...
          formHorizontal={true}
          onPress={(value) => {this.setState({existencias:value})}}/>
          </View>
          :
